...
def update_all_articles():
    ...
    # 获取所有文章
    cursor.execute("SELECT id, url, publish_date FROM articles")
    articles = cursor.fetchall()
    
    updated = 0
    failed = 0
    
    for article_id, url, old_date in articles:
        print(f"[{updated + failed + 1}/{len(articles)}] 更新文章 {article_id}...")
...
if __name__ == "__main__":
    print("=" * 60)
    print("  更新所有文章的发布时间为真实时间")
    print("=" * 60)
    print()
    
    print("这将遍历所有文章并重新抓取发布时间，可能需要几分钟。")
    print("开始执行...")
    update_all_articles()
