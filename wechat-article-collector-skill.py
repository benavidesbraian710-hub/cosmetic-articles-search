...
COORDS = {
    ...
    'close_search': (459, 58),            # 关闭搜索窗口
    'close_account': (528, 57),           # 关闭账号窗口
}
...
def activate_wechat():
    """激活微信窗口到最前台"""
    # 使用 open -a WeChat 激活（比 osascript 更可靠）
    ...
    result = subprocess.run([
        'osascript', '-e',
        'tell application "System Events" to return frontmost of process "WeChat"'
    ], capture_output=True, text=True)
    
    if 'true' in result.stdout.lower():
        print("  ✅ 微信已激活到前台")
    else:
        print("  ⚠️  微信可能未完全激活，继续执行...")
    
    # 方法3: 发送点击确保窗口在最前
    subprocess.run([
        'osascript', '-e',
        'tell application "System Events" to tell process "WeChat" to click at {100, 100}'
    ], capture_output=True)
